...
static const struct linop_s* sense_nc_init(const long max_dims[DIMS], const long map_dims[DIMS], const complex float* maps, const long ksp_dims[DIMS], const long traj_dims[DIMS], const complex float* traj, struct nufft_conf_s conf, bool use_gpu, bool low_mem, struct operator_s** precond_op)
{
...
int main_pics(int argc, char* argv[])
{
	...
	if (NULL == traj_file)
		...
		forward_op = sense_nc_init(max_dims, map_dims, maps, ksp_dims, traj_dims, traj, nuconf, use_gpu, low_mem, (struct operator_s**)&precond_op);
